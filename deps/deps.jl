...
@static Sys.iswindows() ?
	...
	(
		Sys.isapple() ? libnetcdf = split(readlines(pipeline(`otool -L $gmtlib`, `grep libnetcdf`))[1])[1] :
		(
		    Sys.isunix() ? libnetcdf = split(readlines(pipeline(`ldd $gmtlib`, `grep libnetcdf`))[1])[3] :
